...
type Settings = {
  ...
  todoDailyReminderEnabled: boolean;
  todoDailyReminderTimes: string[];
};
...
declare global {
  interface Window {
    MachAndroid?: {
      ...
      syncTodoDailyReminders?: (todosJson: string, timesJson: string) => void;
      syncQuestReminder?: (enabled: boolean, time: string, title: string, detail: string, date: string, done: boolean) => void;
...
const initialSettings: Settings = {
  ...
  questRewardCap: 30,
  questReminderTime: "10:00",
  todoDailyReminderEnabled: false,
  todoDailyReminderTimes: ["10:00", "15:00", "19:00"],
};
...
export default function App() {
  ...
  const [powerNapMinutes, setPowerNapMinutes] = useState(20);
  ...
  useEffect(() => {
    if (!hydrated) return;
    try { window.MachAndroid?.syncTodoDailyReminders?.(JSON.stringify(todos), JSON.stringify(settings.todoDailyReminderEnabled ? settings.todoDailyReminderTimes : [])); } catch { /* Browser preview */ }
  }, [hydrated, todos, settings.todoDailyReminderEnabled, settings.todoDailyReminderTimes]);
  useEffect(() => {
    if (!hydrated) return;
  ...
  useEffect(() => {
    if (!hydrated || !dailyQuest) return;
    try { window.MachAndroid?.syncQuestReminder?.(questProgress.enabled, settings.questReminderTime, dailyQuest.title, dailyQuest.detail, todayKey, questDoneToday); } catch { /* Browser preview */ }
  }, [hydrated, questProgress.enabled, settings.questReminderTime, dailyQuest, todayKey, questDoneToday]);
  ...
  function startPowerNap(minutes: number) {
    const wake = new Date(Date.now() + minutes * 60_000);
    const alarm: Alarm = { ...defaultDraft(settings), id: uid("powernap"), createdAt: Date.now(), qrToken: "wachwerk-personal-code", time: `${pad(wake.getHours())}:${pad(wake.getMinutes())}`, date: localDate(wake), label: `Powernap · ${minutes} Min.`, source: "manual", bedtimeReminderEnabled: false, gentleWake: false, snoozeEnabled: false };
    setAlarms(current => [...current, alarm]);
    showToast(`Powernap läuft · Wecker um ${alarm.time}`);
  }
  function applyCycle() {
  ...
  function renderAlarms() {
    return <><ScreenHeader eyebrow="AUFSTEHEN" title="Deine Wecker" action="＋" onAction={() => openNewAlarm()} />
      <section className="powernap-card"><div><span className="overline">SCHNELLER POWER-NAP</span><h3>Kurze Pause, klarer Kopf</h3><p>20 Minuten sind voreingestellt – kurz genug, um meistens ohne Schlafträgheit aufzuwachen.</p></div><div className="powernap-actions">{[10, 20, 30].map(minutes => <button type="button" className={powerNapMinutes === minutes ? "selected" : ""} key={minutes} onClick={() => { setPowerNapMinutes(minutes); startPowerNap(minutes); }}>{minutes} Min.</button>)}</div><div className="powernap-custom"><label>Eigene Dauer<input aria-label="Eigene Powernap-Dauer in Minuten" type="number" min="5" max="180" value={powerNapMinutes} onChange={event => setPowerNapMinutes(Math.max(5, Math.min(180, Number(event.target.value) || 5)))} /></label><button type="button" onClick={() => startPowerNap(powerNapMinutes)}>Starten</button></div></section>
  ...
  function renderSettings() {
    ...
    return <><ScreenHeader eyebrow="MACH." title="Einstellungen" /><section className="settings-card"><label className="field-label" htmlFor="profile-name">Dein Name</label><input id="profile-name" type="text" placeholder="Optional" value={settings.name} onChange={event => setSettings(current => ({ ...current, name: event.target.value }))} /><label className="field-label" htmlFor="app-font">Schrift in der App</label><select id="app-font" value={settings.appFont} onChange={event => setSettings(current => ({ ...current, appFont: event.target.value as Settings["appFont"] }))}><option value="modern">Modern</option><option value="rounded">Weich & rund</option><option value="classic">Klassisch</option></select><label className="field-label">Farbpalette</label><div className="palette-options" role="group" aria-label="Farbpalette">{([{ id: "classic", name: "Original", colors: ["#06131f", "#c9dcf8", "#9bf5b1", "#ffd347"] }, { id: "solar", name: "Sonnenwärme", colors: ["#003049", "#d62828", "#f77f00", "#fcbf49", "#eae2b7"] }, { id: "dusk", name: "Abendruhe", colors: ["#191629", "#c2b2f1", "#90cdb7", "#edd9bc"] }] as const).map(palette => <button type="button" key={palette.id} aria-pressed={settings.palette === palette.id} className={settings.palette === palette.id ? "selected" : ""} onClick={() => setSettings(current => ({ ...current, palette: palette.id }))}><span className="palette-swatches">{palette.colors.map(color => <i key={color} style={{ background: color }} />)}</span><strong>{palette.name}</strong><span className="palette-check">{settings.palette === palette.id ? "✓" : ""}</span></button>)}</div></section>
      ...
      <section className="settings-card"><span className="overline">TAGESERINNERUNGEN FÜR TO-DOS</span><div className="switch-row"><div><strong>Offene Aufgaben erinnern</strong><small>Zeigt dir über den Tag hinweg, was noch offen ist</small></div><Toggle on={settings.todoDailyReminderEnabled} label="Tageserinnerungen für To-dos" onClick={() => setSettings(current => ({ ...current, todoDailyReminderEnabled: !current.todoDailyReminderEnabled }))} /></div>{settings.todoDailyReminderEnabled && <><div className="daily-reminder-times">{settings.todoDailyReminderTimes.map((time, index) => <TimePicker key={index} label={`To-do-Erinnerung ${index + 1}`} value={time} onChange={value => setSettings(current => ({ ...current, todoDailyReminderTimes: current.todoDailyReminderTimes.map((item, itemIndex) => itemIndex === index ? value : item) }))} />)}</div><p className="field-help">Drei kurze Hinweise öffnen direkt deine offenen To-dos. Erledigte Aufgaben werden nicht genannt.</p></>}</section>
      ...
      <section className="settings-card"><span className="overline">TAGESQUESTS & BELOHNUNGEN</span><div className="switch-row"><div><strong>Tägliche Überraschungsquest</strong><small>Eine lokale Aufgabe pro Kalendertag</small></div><Toggle on={questProgress.enabled} label="Tagesquests" onClick={() => setQuestProgress(current => ({ ...current, enabled: !current.enabled }))} /></div>{questProgress.enabled && <><label className="field-label">Quest-Erinnerung um</label><TimePicker label="Quest-Erinnerung" value={settings.questReminderTime} onChange={value => setSettings(current => ({ ...current, questReminderTime: value }))} /><p className="field-help">Erinnert dich einmal täglich an deine noch offene Tagesquest.</p></>}<label className="field-label" htmlFor="quest-difficulty">Schwierigkeit</label><select id="quest-difficulty" value={questProgress.difficulty} onChange={event => setQuestProgress(current => ({ ...current, difficulty: event.target.value as QuestProgress["difficulty"] }))}><option value="mixed">Gemischt</option><option value="easy">Leicht</option><option value="medium">Mittel</option><option value="hard">Anspruchsvoll</option></select><div className="number-grid"><NumberField label="Bonus je 100 XP" value={settings.questRewardMinutes} min={0} max={60} onCommit={value => setSettings(current => ({ ...current, questRewardMinutes: value }))} /><NumberField label="Bonus-Limit" value={settings.questRewardCap} min={0} max={240} onCommit={value => setSettings(current => ({ ...current, questRewardCap: value }))} /></div><p className="field-help">XP, Serie und Bonusminuten bleiben ausschließlich auf diesem Handy und funktionieren ohne Account.</p></section>
  ...
  function renderAlarmModal() {
    ...
    const bedtimeChoices = [4, 5, 6].map(cycles => ({ cycles, time: formatMinutes(timeToMinutes(alarmDraft.time) - cycles * settings.cycleMinutes - settings.fallAsleepMinutes) }));
